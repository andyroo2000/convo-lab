...
const StudyBrowseCardActions = ({ controller }: Pick<StudyBrowseDetailProps, 'controller'>) => {
...
const StudyBrowseSelectedCard = ({
  controller,
  cardAuthoringCapabilities,
}: StudyBrowseDetailProps) => {
  const { t } = useTranslation('study');
  const {
    actionErrorMessage,
    closePreview,
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="break-words text-sm text-gray-500">
          {t('browse.queue')}:{' '}
          <span className="font-medium text-gray-700">{selectedCard.state.queueState}</span>
        ...
        <StudyBrowseCardActions controller={controller} />
      </div>
      ...
      {actionErrorMessage ? <p className="text-sm text-red-600">{actionErrorMessage}</p> : null}
...
const StudyBrowseSelectedDetail = ({
  controller,
  cardAuthoringCapabilities,
}: StudyBrowseDetailProps) => {
  const { t } = useTranslation('study');
  const { selectedCardId, selectedCardStats, selectedDetail, selectCard } = controller;

  if (!selectedDetail) return <p className="text-gray-500">{t('browse.selectNote')}</p>;
