...
- **[docs]** MVP content topics and pricing decisions - finalized 3 dialogue topics (Meeting Someone New, At a Café/Restaurant, Making Weekend Plans) and 1 audio course topic (Travel & Transportation) for pre-generated sample content; set paid tier pricing at $10/month with 20 items/week limit; defined free tier limits (2 dialogues + 1 audio course lifetime); decided to replace "native language" terminology with "Primary Language" and remove country flags in favor of language names only
...
- **[fix]** useEffect dependency array warning in VerifyEmailPage

### Added

- **[feat]** Client-side Anki-style scheduling preview with TTS audio for SRS flashcards - implemented instant scheduling interval previews showing when cards will be due next (e.g., <1m, 10m, 3d, 7d) displayed above each rating button; computation happens client-side using ts-fsrs library with zero network latency via useMemo; added TTS audio generation fallback using Google Cloud TTS with voice mappings for all languages ensuring every card has audio even when course audio extraction fails; generates TTS for full sentences (not just vocabulary words); fixed course generation to map vocabulary to full dialogue sentences instead of standalone word units (added length check: unit.text.length > vocab.textL2.length); added sentenceTranslationL1 field to Card schema; enhanced FlashCard to conditionally show sentence translations only when different from vocabulary word; fixed vocabulary highlighting to use className prop directly; fixed ESLint errors in CoursePage (nested ternaries, accessibility, keyboard handlers)
