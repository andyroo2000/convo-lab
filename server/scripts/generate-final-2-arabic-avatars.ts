...
async function main() {
  console.log('Arabic Avatar Generator - Final 2 Avatars');
  console.log('=========================================\n');
  console.log('Current status: 4 of 6 avatars complete');
  console.log('Generating the last 2 male avatars...\n');

  let successCount = 0;
  let failCount = 0;

  for (const config of FINAL_ARABIC_AVATARS) {
    try {
  ...
  if (successCount === 2) {
    ...
    console.log(`\n⚠️  ${2 - successCount} avatar(s) still need to be generated`);
    console.log('Please wait for rate limit to reset and run this script again.');
  }
}
