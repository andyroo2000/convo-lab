...
import {
  generateNarrowListeningPack,
  StoryPack,
} from '../../../services/narrowListeningGenerator.js';
...
describe('narrowListeningGenerator', () => {
  const mockStoryPack: StoryPack = {
    ...
    versions: [
      ...
      },
      {
        ...
        segments: [
          {
            targetText: '今日カフェに行きます。',
            englishTranslation: 'I am going to a cafe today.',
          },
      ...
      },
      {
        ...
        segments: [
          {
            targetText: '明日カフェに行くつもりです。',
            englishTranslation: 'I plan to go to a cafe tomorrow.',
          },
  ...
  describe('generateNarrowListeningPack', () => {
    it('should generate a story pack for Japanese', async () => {
      const result = await generateNarrowListeningPack('A person visiting a cafe', 'ja', 'N4', 3);
    ...
    it('should include topic in the prompt', async () => {
      await generateNarrowListeningPack('Shopping at a convenience store', 'ja', 'N4', 3);
    ...
    it('should include proficiency level description', async () => {
      await generateNarrowListeningPack('Test topic', 'ja', 'N5', 3);
    ...
    it('should include version count in the prompt', async () => {
      await generateNarrowListeningPack('Test topic', 'ja', 'N4', 5);
    ...
    it('should include grammar focus when provided', async () => {
      await generateNarrowListeningPack('Test topic', 'ja', 'N4', 3, 'Focus on particle は vs が');
    ...
    it('should strip markdown code fences from response', async () => {
      ...
      const result = await generateNarrowListeningPack('Test topic', 'ja', 'N4', 3);
    ...
    it('should throw error for invalid story pack structure', async () => {
      ...
      await expect(generateNarrowListeningPack('Test topic', 'ja', 'N4', 3)).rejects.toThrow(
        'Invalid story pack structure'
      );
    });
    ...
    it('should throw error when Gemini fails', async () => {
      ...
      await expect(generateNarrowListeningPack('Test topic', 'ja', 'N4', 3)).rejects.toThrow(
        'Failed to generate narrow listening pack'
      );
    });
  });
  ...
  describe('language-specific behavior', () => {
    describe('Japanese', () => {
      it('should include Japanese variation types in prompt', async () => {
        await generateNarrowListeningPack('Test topic', 'ja', 'N4', 3);
      ...
      it('should map N3 to intermediate level', async () => {
        await generateNarrowListeningPack('Test topic', 'ja', 'N3', 3);
      ...
      it('should map N2/N1 to advanced level', async () => {
        await generateNarrowListeningPack('Test topic', 'ja', 'N1', 3);
