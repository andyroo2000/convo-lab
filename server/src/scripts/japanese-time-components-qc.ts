...
const DEFAULT_VOICE_ID = 'fishaudio:b3e9710c629a472f8224e1c4975a869e'; // Ohtani-san

type Provider = 'fishaudio' | 'google' | 'polly' | 'azure';

type TimeComponentEntry = {
  id: string;
  category: 'time_12h_part1' | 'time_24h_part1' | 'minute';
  label: string;
  text: string;
  relativePath: string;
};
...
function buildEntries(): TimeComponentEntry[] {
  const entries: TimeComponentEntry[] = [];
  const anchorDate = new Date(2026, 1, 13, 0, 0, 0, 0);

  for (let hour24 = 0; hour24 < 24; hour24 += 1) {
    const d = new Date(anchorDate);
    d.setHours(hour24, 0, 0, 0);

    const reading12h = generateJapaneseDateTimeReading(d, { hourFormat: '12h' });
    const reading24h = generateJapaneseDateTimeReading(d, { hourFormat: '24h' });
    const period = reading12h.parts.periodKana === 'ごご' ? 'gogo' : 'gozen';
    const hour12 = twoDigits(hour24 % 12 || 12);
    const hh = twoDigits(hour24);

    entries.push({
      id: `time_12h_part1_${period}_${hour12}`,
      category: 'time_12h_part1',
      label: `12h part1 (${period.toUpperCase()} ${hour12})`,
      text: `${reading12h.parts.periodKana || ''} ${reading12h.parts.hourKana}`.trim(),
      relativePath: `time/12h/part1/${period}-${hour12}.mp3`,
    });

    entries.push({
      id: `time_24h_part1_${hh}`,
      category: 'time_24h_part1',
      label: `24h part1 (${hh})`,
      text: reading24h.parts.hourKana,
      relativePath: `time/24h/part1/${hh}.mp3`,
    });
  }

  for (let minute = 0; minute < 60; minute += 1) {
    const d = new Date(anchorDate);
    d.setHours(9, minute, 0, 0);
    const reading24h = generateJapaneseDateTimeReading(d, { hourFormat: '24h' });
    const mm = twoDigits(minute);

    entries.push({
      id: `minute_${mm}`,
      category: 'minute',
      label: `Minute ${mm}`,
      text: reading24h.parts.minuteKana,
      relativePath: `time/minute/${mm}.mp3`,
    });
  }
...
<body>
  ...
  <script>
    ...
    const storageKey = 'convolab-time-qc-' + voiceId;
    ...
    const render = () => {
      ...
      entries.forEach((entry) => {
        ...
        idTd.innerHTML = '<span class="pill">' + entry.id + '</span>';
        tr.appendChild(idTd);
    ...
    document.getElementById('exportCsv').addEventListener('click', () => {
      const header = 'id,category,text,relativePath,status,notes';
      const lines = entries.map((entry) => {
        const rowState = state[entry.id] || { status: 'pending', notes: '' };
        const safe = (v) => '"' + String(v || '').replaceAll('"', '""') + '"';
        return [entry.id, entry.category, entry.text, entry.relativePath, rowState.status, rowState.notes || ''].map(safe).join(',');
      });
...
async function main() {
  ...
  const outDir = outDirArg || `/Users/andrewlandry/Desktop/time-components-qc/${voiceSlug}`;

  if (provider === 'azure') {
    throw new Error(
      'Azure provider is not wired in this repository yet. Add Azure TTS provider first.'
    );
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.log(`[Time Components] OK ${entry.id} -> ${entry.relativePath}`);
    } catch (error) {
      ...
      failed += 1;
      console.log(`[Time Components] FAIL ${entry.id} -> ${reason}`);
    }
  }
  ...
  await fs.writeFile(
    ...
    [
      ...
      `npm run smoke:jp-time-components-qc -- --voice-id "${voiceId}" --out-dir "${outDir}" --defects-json "/path/to/defects.json"`,
      '```',
      '',
  ...
  console.log('\nTime component generation complete.');
...
main().catch((error) => {
  console.error('Time component generation failed:', error);
  process.exitCode = 1;
});
