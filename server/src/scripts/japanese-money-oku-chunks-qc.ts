...
// Oku-chunk clips are spoken as full `Xおく` values, so we materialize at 100,000,000-unit scale.
const chunkToKana = (chunkValue: number): string => buildMoneyReading(chunkValue * 100_000_000).kana;

const toRelativePath = (chunkValue: number): string =>
  `money/oku-chunk/${String(chunkValue).padStart(4, '0')}.mp3`;
...
async function main() {
  ...
  console.log(`[Money Oku Chunks] Voice: ${voiceId}`);
  console.log(`[Money Oku Chunks] Chunk range: ${chunkStart}..${chunkEnd}`);
  console.log(`[Money Oku Chunks] Output: ${outDir}`);
  console.log(`[Money Oku Chunks] Upload to GCS: ${uploadGcs ? 'yes' : 'no'}`);
  console.log(`[Money Oku Chunks] Only missing: ${onlyMissing ? 'yes' : 'no'}`);
  ...
  for (let chunkValue = chunkStart; chunkValue <= chunkEnd; chunkValue += 1) {
    ...
    try {
      ...
      console.log(`[Money Oku Chunks] FAIL ${relativePath} -> ${reason}`);
    }

    const processed = chunkValue - chunkStart + 1;
    if (processed % 25 === 0 || chunkValue === chunkEnd) {
      console.log(
        `[Money Oku Chunks] ${processed}/${total} generated=${generated} skipped=${skipped} uploaded=${uploaded} failed=${failed}`
      );
    }
  }
  ...
  await fs.writeFile(
    path.join(outDir, 'money', 'oku-chunk-manifest.json'),
    JSON.stringify(manifest, null, 2)
  );

  console.log('[Money Oku Chunks] Complete');
  console.log(
    `[Money Oku Chunks] Totals -> generated: ${generated}, skipped: ${skipped}, uploaded: ${uploaded}, failed: ${failed}, total: ${total}`
  );
}

main().catch((error) => {
  console.error('[Money Oku Chunks] Fatal error:', error);
  process.exitCode = 1;
});
