...
type VerbConjugationCatalogEntry = {
  id: string;
  verbId: string;
...
type Manifest = {
  ...
  entries: VerbConjugationCatalogEntry[];
...
async function loadVerbConjugationCatalog(): Promise<VerbConjugationCatalogEntry[]> {
  // Raw cross-workspace path required because the client package isn't published as
  // a workspace dependency of the server. tsx can resolve .ts imports directly.
  const moduleUrl = new URL(
    '../../../client/src/features/tools/japaneseVerbs/logic/verbConjugation.ts',
    import.meta.url
  );
  const mod = (await import(moduleUrl.href)) as {
    buildVerbConjugationCatalog?: () => VerbConjugationCatalogEntry[];
  };

  if (typeof mod.buildVerbConjugationCatalog !== 'function') {
    throw new Error('buildVerbConjugationCatalog export was not found in verbConjugation.ts');
  }

  const entries = mod.buildVerbConjugationCatalog();

  // Runtime shape check — catches drift between the local type and the client export
  const first = entries[0];
  if (first) {
    const requiredKeys: (keyof VerbConjugationCatalogEntry)[] = [
      'id',
      'verbId',
      'conjugationId',
      'text',
    ...
    if (missing.length > 0) {
      throw new Error(
        `VerbConjugationCatalogEntry shape mismatch — missing keys: ${missing.join(', ')}`
      );
    }
  }
...
function buildQcHtml(
  ...
  const escapedEntries = JSON.stringify(entries).replace(/<\//g, '<\\/');
...
<head>
  ...
  <title>Japanese Verb Conjugation QC</title>
  <style>
...
<body>
  <h1>Japanese Verb Conjugation QC</h1>
  ...
  <table id="qcTable">
    <thead>
      <tr>
        ...
        <th style="width: 100px;">Verb</th>
        <th style="width: 140px;">Conjugation</th>
        <th>Text (Script)</th>
        <th>Text (Kana)</th>
  ...
  <script>
    ...
    const storageKey = 'convolab-verb-qc-' + voiceId;
    ...
    const render = () => {
      ...
      entries.forEach((entry) => {
        ...
        if (filterDefective && rowState.status !== 'defective') return;
        ...
        verbTd.textContent = entry.verbId;
        tr.appendChild(verbTd);

        const conjTd = document.createElement('td');
        conjTd.textContent = entry.conjugationId;
        tr.appendChild(conjTd);
    ...
    const toDefectsPayload = () => {
      const defectiveIds = entries
        ...
        .map((entry) => entry.id);
      return { voiceId, defectiveIds, exportedAt: new Date().toISOString() };
    };
    ...
    document.getElementById('exportCsv').addEventListener('click', () => {
      const header = 'id,verbId,conjugationId,text,kanaText,relativePath,status,notes';
      const lines = entries.map((entry) => {
        const rowState = state[entry.id] || { status: 'pending', notes: '' };
        const safe = (v) => '"' + String(v || '').replaceAll('"', '""') + '"';
        return [entry.id, entry.verbId, entry.conjugationId, entry.text, entry.kanaText, entry.relativePath, rowState.status, rowState.notes || ''].map(safe).join(',');
      });
...
async function main() {
  ...
  const outDir =
    outDirArg || path.resolve('tmp/verb-conjugations-qc/google-kento-professional');
  ...
  const entries = await loadVerbConjugationCatalog();
  console.log(`Loaded ${entries.length} verb conjugation entries.`);
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const audio = await google.synthesizeSpeech({
        text: entry.kanaText,
        voiceId,
        languageCode: 'ja-JP',
        speed: 1.0,
      ...
      console.log(`[Verb Conjugation] OK ${entry.id} -> ${entry.relativePath}`);
    } catch (error) {
      ...
      failed += 1;
      console.log(`[Verb Conjugation] FAIL ${entry.id} -> ${reason}`);
    }
  }
  ...
  console.log('\nJapanese verb conjugation audio generation complete.');
...
main().catch((error) => {
  console.error('Japanese verb conjugation audio generation failed:', error);
  process.exitCode = 1;
});
