...
import { isLearningOsImageGenerationProxyEnabled } from '../config/imageGenerationRouting.js';
...
type ImageHandler = (req: AuthRequest, res: Response, next: NextFunction) => void | Promise<void>;

const routeImageGeneration = (
  learningOsHandler: ImageHandler,
  expressHandler: ImageHandler
): ImageHandler => {
  const handler: ImageHandler = (req, res, next) =>
    (isLearningOsImageGenerationProxyEnabled() ? learningOsHandler : expressHandler)(
      req,
      res,
      next
    );
...
router.post(
  '/generate',
  routeImageGeneration(generateLearningOsImages, async (req: AuthRequest, res, next) => {
    try {
...
router.get(
  '/job/:jobId',
  routeImageGeneration(showLearningOsImageJob, async (req: AuthRequest, res, next) => {
    try {
