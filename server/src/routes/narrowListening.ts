...
router.get('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!pack) {
      throw new AppError(
        i18next.t('server:content.notFound', { type: 'Narrow listening pack' }),
        404
      );
    }
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      if (targetLanguage === 'ja') {
        ...
        if (!cefrLevel) {
          throw new AppError(
            i18next.t('server:validation.cefrRequired', { language: 'Spanish' }),
            400
          );
        }
        ...
        if (!cefrLevel) {
          throw new AppError(
            i18next.t('server:validation.cefrRequired', { language: 'French' }),
            400
          );
        }
        const validCefrLevels = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
        ...
        if (!cefrLevel) {
          throw new AppError(
            i18next.t('server:validation.cefrRequired', { language: 'Arabic' }),
            400
          );
        }
        const validCefrLevels = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
