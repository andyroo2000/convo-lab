  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const itemsWithAudio = session.items.map((item, itemIndex) => {
        if (item.type === 'meaning_match' && item.sentencePair) {
          const audioIndexA = textsToSynthesize.findIndex(
            (t) => t.itemIndex === itemIndex && t.type === 'A'
          );
          const audioIndexB = textsToSynthesize.findIndex(
            (t) => t.itemIndex === itemIndex && t.type === 'B'
          );
          return {
            ...item,
        ...
        const audioIndex = textsToSynthesize.findIndex(
          (t) => t.itemIndex === itemIndex && t.type === 'main'
        );
        return {
          ...item,
