...
import { isLearningOsAudioGenerationProxyEnabled } from '../config/audioGenerationRouting.js';
...
type AudioHandler = (req: AuthRequest, res: Response, next: NextFunction) => void | Promise<void>;

const routeAudioGeneration = (
  learningOsHandler: AudioHandler,
  expressHandler: AudioHandler
): AudioHandler => {
  const handler: AudioHandler = (req, res, next) =>
    (isLearningOsAudioGenerationProxyEnabled() ? learningOsHandler : expressHandler)(
      req,
      res,
      next
    );
...
router.post(
  '/generate',
  routeAudioGeneration(generateLearningOsAudio, async (req, res, next) => {
    try {
...
router.post(
  '/generate-all-speeds',
  routeAudioGeneration(generateAllSpeedsLearningOsAudio, async (req, res, next) => {
    try {
...
router.get(
  '/job/:jobId',
  routeAudioGeneration(showLearningOsAudioJob, async (req, res, next) => {
    try {
