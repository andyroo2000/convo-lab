...
- imagePrompt must use this visual style: ${IMAGE_PROMPT_STYLE}.
...
- Include answer.notes on every candidate. ${STUDY_CARD_NOTES_GUIDANCE} Include example sentence fields only when they add value beyond the target sentence. When adding answer.sentenceJp and answer.sentenceEn, make them short and natural but slightly specific, with a concrete situation, time, place, or speaker intention. Avoid generic dictionary-style examples like 今日は曇りです。
