...
Exact component commits live in `.github/release-integration/components.json`. Updating a pin is a reviewed repository change. The workflow creates a uniquely named `integration/compatibility-<run>-<attempt>` branch at the pinned web commit, runs the gates, and deletes that exact branch even when a gate fails. It never opens or merges a pull request.
